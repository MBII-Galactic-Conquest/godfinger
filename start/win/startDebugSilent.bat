...
REM Kill any orphaned MBIII.x86.exe processes if autoclient.cfg contains 1
findstr /B /C:"1" ..\autoclient.cfg >nul 2>&1
if %errorlevel%==0 (
    echo Killing orphaned MBIII.x86.exe processes...
    start /wait powershell -WindowStyle Normal -Command "Stop-Process -Name 'MBIII.x86' -Force -ErrorAction SilentlyContinue"
)

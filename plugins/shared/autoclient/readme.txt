...
2. MBII game client (MBIII.x86.exe) accessible on the same machine
3. Dedicated server (MBIIIded.x86.exe) must be running
...

{
    "enabled": true,
    "maxFakeClients": 4,
    "clientExecutablePath": "C:/Path/To/GameData/MBIII.x86.exe",
    "serverIP": "127.0.0.1",
    "serverPort": "29070",
    "serverProcessName": "MBIIIded.x86.exe",
    "launchDelay": 5.0,
...
PARAMETERS:
  ...
  clientExecutablePath  - Full path to MBIII.x86.exe game client
...
The plugin checks that MBIIIded.x86.exe is running before spawning any clients.
...
Usage:
  python spawnClient.py --exe "C:/Path/To/MBIII.x86.exe" --ip "127.0.0.1" --count 3

Arguments:
  --exe PATH        Path to MBIII.x86.exe (required)
...
A: Check:
   1. clientExecutablePath points to a valid MBIII.x86.exe
   2. MBIIIded.x86.exe server process is running

...
Windows-only plugin - requires MBIIIded.x86.exe to be running.
...
CONFIG_FALLBACK = """{
    "enabled": true,
    "maxFakeClients": 8,
    "clientExecutablePath": "C:/Path/To/MBIII.x86.exe",
    "serverIP": "127.0.0.1",
    "serverPort": "29070",
    "launchDelay": 15,
...
class AutoClientPlugin:
    ...
    def _IsServerRunning(self) -> bool:
        ...
        try:
            ...
            process_name = self.config.cfg.get("serverProcessName", "MBIIIded.x86.exe")
            result = subprocess.run(
    ...
    def Finish(self):
        ...
        # Use PowerShell to force kill all MBIII.x86.exe processes
        # This is more reliable than trying to kick/terminate individually
        try:
            subprocess.run(
                ["powershell", "-Command", "Stop-Process -Name 'MBIII.x86' -Force -ErrorAction SilentlyContinue"],
                creationflags=subprocess.CREATE_NO_WINDOW,
                timeout=10
            )
            Log.info("AutoClient cleanup: killed all MBIII.x86.exe processes")
        except Exception as e:
